...
class CorrelationRegistryTest extends TestCase
{
    ...
    public function test_callback_exception_without_handler_emits_fallback_telemetry_and_continues(): void
    {
        ...
        $completed = false;
        $pending->onComplete(function (): void {
            throw new \RuntimeException('missing-handler-boom');
        });
        $pending->onComplete(function (?Throwable $e, ?Response $r) use (&$completed): void {
            $completed = $e === null && $r?->isSuccess() === true;
        });
        ...
        $this->assertStringContainsString('"server_key":"nodeA"', $contents);
        $this->assertStringContainsString('"action_id":"nodeA:1:1"', $contents);
